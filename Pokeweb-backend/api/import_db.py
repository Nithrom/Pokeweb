...
=================================================
Instrucciones:
  1. Copia tu pokemon_db.json a la carpeta Pokeweb-backend/api/
...
def main():
    ...
    if not os.path.exists(JSON_PATH):
        ...
        print('Copia pokemon_db.json a Pokeweb-backend/api/ y vuelve a ejecutar.')
        sys.exit(1)
    ...
    for name, pdata in sorted(pokemon_data.items(), key=lambda x: x[1]['id']):
        ...
        cur.execute(
            '''INSERT INTO pokemon (id, name, name_es, sprite_url, evo_family_id)
               VALUES (%s,%s,%s,%s,%s)
               ON DUPLICATE KEY UPDATE sprite_url=VALUES(sprite_url),
               evo_family_id=VALUES(evo_family_id)''',
            (pid, name, name_es, sprite, evo_fam))
